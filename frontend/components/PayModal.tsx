...
export default function PayModal({
  ...
  return (
    ...
    >
      ...
      <div ref={containerRefModal} className="   bg-white overflow-auto py-4 px-6  sm:px-8 rounded-lg relative max-[420px]:px-2 w-full sm:w-10/12 md:w-9/12 lg:w-8/12 2xl:w-7/12">
        ...
        <section className="">
          <section className="bg-gray-300 p-2 rounded-lg text-center">
            <p>تعداد کل: {data?.allCount}</p>
            <p>مبلغ کل: {data?.allPrice}</p>
          </section>
          <DefaultButton
            content="پرداخت"
            className="w-full max-[280px]:h-6  h-10 bg-sky-500 rounded-lg text-white mt-3"
          />
        </section>
      </div>
      </div>
