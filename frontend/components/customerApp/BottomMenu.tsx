...
export default function BottomMenu() {
  const { totalNumber, totalNumberRegisterdOrders } = useOrderCardContext();

  return (
    <div className=" h-12 pt-2 bg-sky-500 border-t sticky bottom-0 flex items-center justify-between sm:justify-around px-8  mx-auto border border-sky-500  shadow-xl sm:w-5/6 md:w-5/6 lg:w-4/6  ">
      ...
      <Link href="/application/order" className="relative">
        ...
        <div className="size-5 rounded-full text-white flex items-center justify-center bg-sky-700 absolute right-[3px] -top-3 text-sm ">
          {totalNumber}
        </div>
      </Link>
    </div>
