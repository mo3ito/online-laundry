    ...
    <section className="w-full h-max bg-sky-500 fixed top-0 left-0 flex flex-col items-center justify-between">
      <header className="flex justify-between items-center w-full px-8 py-3">
        <div className=" ">
          ...
          <p className="text-xl text-white">{`${infos?.name || ""} ${infos?.last_name || ""}`}</p>
          </div>
          <button onClick={()=>setIsShowLogoutModal(true)} className="flex justify-center items-center gap-x-2 mt-1 mr-1">
        <svg className="size-6 fill-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M5 11H13V13H5V16L0 12L5 8V11ZM3.99927 18H6.70835C8.11862 19.2447 9.97111 20 12 20C16.4183 20 20 16.4183 20 12C20 7.58172 16.4183 4 12 4C9.97111 4 8.11862 4.75527 6.70835 6H3.99927C5.82368 3.57111 8.72836 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C8.72836 22 5.82368 20.4289 3.99927 18Z"></path></svg>
      ...
      <Modal messageContent="آیا از خروج اطمینان دارید؟" isShowModal={isShowLogoutModal} setIsShowModal={setIsShowLogoutModal} confirmOnClick={logoutHandler} />
    </section>
  );
}
