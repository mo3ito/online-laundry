...
export default function Page() {
  ...
  return (
    <div className="container mx-auto px-6 py-3">
                ...
                <div className="size-full flex items-center justify-center flex-col">
                  <figure className="p-2 size-28 bg-sky-200 rounded-xl -translate-y-3 cursor-pointer">
                    <img
                      ...
                      src={
                        category.image_url || "/images/no-image.jpg"}
      ...
      {typeCategory?.map((item: InformationClothingsItemProps, index: number) => (
  <div className="w-full h-max mt-5" key={item._id}>
    <div
      onClick={() => toggleDetails(index)}
      className="w-full h-max flex justify-between items-center cursor-pointer bg-sky-200 rounded-lg"
    >
      <figure className="p-2 rounded-xl flex items-center gap-x-4">
        <img
          src={item.image_url || "/images/no-image.jpg"}
          className="rounded-xl bg-red-200 size-28"
          alt={`عکس ${item.type}`}
        />
    ...
    <section
      className={`${showDetails[index] ? "block" : "hidden"} w-full h-max`}
    >
      <table className="max-[280px]:text-xs table-auto w-full border-collapse border border-gray-400">
        <thead>
        ...
        <tbody className="text-center">
          <tr>
            ...
            <td className="border border-gray-300 px-4 py-2">
              {item.services.map((service) => (
                <div key={service._id} className="my-2">
                  <p className="border-b py-2">{Number(service.price).toLocaleString("en-US")}</p>
                </div>
              ))}
            </td>
          </tr>
